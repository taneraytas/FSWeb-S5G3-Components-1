...
function haberYapici(haberler) {
  ...
  articlesContainer.appendChild(divElement);
  divElement.append(
    titleElement,
    dateElement,
    p1Element,
  ...
  dateElement.classList.add("tarih");
  spanElement.classList.add("expandButton");

  titleElement.textContent = haberler.baslik;
  p1Element.textContent = haberler.ilkParagraf;
  p2Element.textContent = haberler.ikinciParagraf;
  p3Element.textContent = haberler.ucuncuParagraf;
...
data.forEach((item) => {
  articlesContainer.appendChild(haberYapici(item));
});
